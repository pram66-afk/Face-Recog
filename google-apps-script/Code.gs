...
function handleRotateToken(body) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][sessionIdCol] === sessionId) {
      const oldToken = data[i][tokenCol];
      
      // Store the old token in cache for 60 seconds to allow pending face scans to finish
      const cache = CacheService.getScriptCache();
      cache.put('prev_token_' + sessionId, oldToken, 60);
...
function handleMarkAttendance(body) {
  ...
  // 2. Validate Token (Rotation Resilience Logic)
  const cache = CacheService.getScriptCache();
  const prevToken = cache.get('prev_token_' + body.sessionId);
  
  const isTokenValid = (session.Token === body.token || (prevToken && prevToken === body.token));

  if (!isTokenValid) {
      console.warn('Token mismatch. Scanned: ' + body.token + ', Current: ' + session.Token + ', Prev: ' + prevToken);
      return jsonResponse({ 
        success: false, 
        error: 'QR code has rotated. If you scanned an old code, please try the new one on screen.', 
        code: 'INVALID_TOKEN' 
      });
  }
  ...
  if (duplicate) {
    return jsonResponse({ success: true, message: 'Attendance already marked.', code: 'DUPLICATE' });
  }
